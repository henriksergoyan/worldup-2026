          ...
          <CardContent className="space-y-2">
            {upcomingMatches.length === 0 ? (
              <div className="rounded-xl border border-white/10 bg-white/[0.02] px-4 py-8 text-center">
                ...
                <p className="mt-2 text-sm font-semibold text-white">Այս պահին գալիք խաղեր չկան</p>
              </div>
            ) : (
        ...
        <Card className="overflow-hidden border-white/10">
          ...
          <CardContent>
            {recentResults.length === 0 ? (
              <div className="flex flex-col items-center justify-center rounded-xl border border-white/10 bg-white/[0.02] px-4 py-8 text-center">
                <div className="text-3xl">⏳</div>
                <p className="mt-2 text-sm text-navy-300">Արդյունքներ դեռ չկան</p>
              </div>
            ) : (
                ...
                {recentResults.map((m) => {
                  ...
                  const stageLabel =
                    isKnockout
                      ? ROUND_LABELS[(m.round as Round) ?? "R32"]
                      : `Խումբ ${m.groupCode}`;
                  const actualScore = m.actualResult
                    ? isKnockout
                      ? {
                  ...
                  return (
                    <Link
                      ...
                      href={`/matches/${m.id}`}
                      className={cn(
                        "block rounded-xl border px-3 py-3 transition sm:p-4",
                        won
                    ...
                    >
                      ...
                      <div className="grid grid-cols-[1fr_auto_1fr] items-center gap-2 sm:gap-4">
                        ...
                        <div className="flex flex-col items-center gap-1.5">
                          {actualScore && isKnockout ? (
                            <KnockoutScoreDisplay score={actualScore} size="sm" />
                          ) : (
                          ...
                          {hasPred ? (
                            ...
                            >
                              Ձերը՝{" "}
                              {predKnockoutScore
                                ? formatKnockoutScoreInline(predKnockoutScore)
                                : `${pred!.normalHomeGoals}–${pred!.normalAwayGoals}`}
                            </span>
                          ) : (
